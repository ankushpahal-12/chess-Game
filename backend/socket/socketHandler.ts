...
import { GameService, getCapturedPieces } from '../services/gameService';
import { Game } from '../models/Game';
import { Chess } from 'chess.js';
import { verifySessionToken, hashToken } from '../utils/session';
import { scheduleForfeitCheck, cancelForfeitCheck } from './timerRegistry';
import { startExpirySweep } from '../jobs/expirySweep';
...
const chatLimits = new Map<string, { count: number; windowStart: number }>();
...
export const setupSocket = (io: Server) => {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    // 1. Create Game
    socket.on('create_game', async ({ nickname, timeControl }: { nickname: string; timeControl: number }) => {
      try {
        const { game, token } = await GameService.createGame(socket.id, nickname, timeControl);
        socket.data.session = verifySessionToken(token);
    ...
    socket.on('join_request', async ({ code, nickname }: { code: string; nickname: string }) => {
      try {
        const upperCode = code.toUpperCase();
        const { game, playerId } = await GameService.submitJoinRequest(upperCode, nickname, socket.id);
    ...
    socket.on('respond_join', async ({ 
      ...
      try {
        const upperCode = code.toUpperCase();
        const game = await Game.findOne({ code: upperCode });
        if (!game) {
        ...
        const { game: activeGame, hostToken, guestToken } = await GameService.acceptJoinRequest(
          upperCode, 
          socket.id, 
          guestSocketId, 
          guestName
        ...
        const hostPayload = verifySessionToken(hostToken);
        const guestPayload = verifySessionToken(guestToken);
    ...
    // 5. Offer Draw
    socket.on('offer_draw', async ({ gameId, playerId }: { gameId: string; playerId: string }) => {
      try {
        const session = socket.data.session;
        if (!session || session.gameId !== gameId || session.playerId !== playerId) {
    ...
    // 6. Respond Draw
    socket.on('respond_draw', async ({ gameId, playerId, accept }: { gameId: string; playerId: string; accept: boolean }) => {
      try {
        const session = socket.data.session;
        if (!session || session.gameId !== gameId || session.playerId !== playerId) {
        ...
        if (accept) {
          const updatedGame = await GameService.agreeDraw(gameId);
          io.to(updatedGame.code).emit('game_over', {
            winner: 'draw',
    ...
    socket.on('resign', async ({ gameId, playerId }: { gameId: string; playerId: string }) => {
      try {
        ...
        const updatedGame = await GameService.resign(gameId, playerId);
        io.to(updatedGame.code).emit('game_over', {
          winner: updatedGame.winner,
          reason: 'resign',
          finalHistory: updatedGame.history
    ...
    // 8. Rejoin Game
    socket.on('rejoin_game', async ({ token }: { token: string }) => {
      try {
        ...
        if (game.status === 'FINISHED') {
          ...
          if (!player || player.sessionTokenHash !== hashToken(token)) {
            return socket.emit('rejoin_failed', { reason: 'invalid_session' });
          }
          socket.data.session = payload;
          socket.join(game.code);
          ...
          return socket.emit('rejoin_success', {
            ...
            turn: new Chess(game.fen).turn() === 'w' ? 'white' : 'black',
        ...
        if (player.sessionTokenHash !== hashToken(token)) {
        ...
        cancelForfeitCheck(game.id, player.color);
    ...
    socket.on('join_spectator', async ({ code }: { code: string }) => {
      try {
        const upperCode = code.toUpperCase();
        const game = await Game.findOne({ code: upperCode });
        if (!game) {
    ...
    socket.on('secure_init', ({ code }: { code: string }) => {
      if (!code) return;
      socket.to(code.toUpperCase()).emit('secure_init');
    });

    socket.on('secure_key', ({ code, publicKey, version }: { code: string; publicKey: ArrayBuffer; version?: number }) => {
      if (!code || !publicKey) return;
      socket.to(code.toUpperCase()).emit('secure_key', { publicKey, version });
    });

    socket.on('secure_payload', ({ code, payload, version }: { code: string; payload: any; version?: number }) => {
    ...
    socket.on('secure_close', ({ code }: { code: string }) => {
      if (!code) return;
    ...
    socket.on('disconnect', async () => {
      ...
      if (!player || player.socketId !== socket.id) return; // Stale socket check
      ...
      scheduleForfeitCheck(game.id, player.color, game.disconnectTimeoutMs, async () => {
        const fresh = await Game.findById(game._id);
  ...
  setInterval(async () => {
    ...
    try {
      const timedOutGames = await GameService.tickTimers();
